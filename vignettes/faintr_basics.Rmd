...
- `extract_cell_draws` provides the posterior draws for one subset of factorial design cells. 
...
Additionally, we can speed up the sampling process by using all available cores:

```{r cpu-options, eval=FALSE}
options(mc.cores = parallel::detectCores())
```

```{r internal-setup, echo=FALSE}
# CRAN only allows 2 cores max
options(mc.cores = 2)
...
extract_cell_draws(fit_brms_politeness, gender == 'M' & context == 'inf') %>% head()
```

We can also use negations and disjunctions to subset design cells:

```{r cell-draws-dummy-misc}
extract_cell_draws(fit_brms_politeness, gender != 'F' | context == 'inf') %>% head()
```
...
extract_cell_draws(fit_brms_politeness, context == 'pol', colname = 'context:pol') %>% head()
```
...
extract_cell_draws(fit_brms_politeness, colname = 'grand_mean') %>% head()
```

We can use the output of `extract_cell_draws` for inspecting and comparing posterior draws visually:

```{r cell-draws-dummy-plot}
draws_gender <- cbind(extract_cell_draws(fit_brms_politeness, gender == 'M', colname = 'male'),
